...
\begin{proof}
    ...
    由\textup{Cauchy}命题知，因为$\{a_n\}$收敛于$A$，即数列$\{\ln{a_n}\}$收敛于$\ln{A}$. \\    
    所以
...
\begin{proposition}[Cauchy命题$\infty$时的特例]
...
\begin{proof}
    ...
    令$A^{(i)}_{n} = a_{(n + 1)p + i} - a_{np + i}$，由\textup{Cauchy}命题知
