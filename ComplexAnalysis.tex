...
\begin{proof}
    ...
    反证法。若存在$a, b \in \mathbb{D}$，$\quad a \neq b$，且
    $$f(a) = a, \quad  f(b) = b$$
    则设$\varphi(z) = \dfrac{a - z}{1 - \overline{a}z}$，$\varphi$将$\mathbb{D}$映射成$\mathbb{D}$，且$\varphi(0) = a $ \\
...
\begin{proposition}
    ...
    其中$\mathbb{D} = \{z \big| |z| < 1\}$，$A$是正实数，那么任意$r \in (0, 1)$
